...
const Result = ( {userData,rating,stdData}) => {
  ...
  const  sopInfo = async (programValues,sop) =>{
    ...
    {
      instructions:
        `I am providing an SOP of a student. Please read through the SOP and generate me a list of the all characteristics that are mentioned and identified in the SOP. After that, calculate the percentage of those which match the program values.
        The program values are ${programValues}. Lastly, please also generate a two line summary of sop. 
        Provide in Json Format like given below:
        {
          "characteristicsInSOP": [],
          "percentageMatchInSOP": "x%",
  ...
  const calculations = async ()=>{
    ...
    for(let i=0;i<stdData.length;i++){
      ...
      if(currentStd.sop!=="No SOP Found!"){
        ...
        if (typeof sopScore !== 'object') {
          console.log(typeof sopScore)
          sopScore = "NA"; // this line is not necessary but good to it's validate
        }
      }
      ...
      console.log("Lol scores",lorScore?.percentageMatchInLOR)
...
const openPDFInNewTab = (item) => {
    ...
    img.onload = function () {
        ...
        // Date of Birth
        doc.text(`Date of Birth: ${item.dob}`, 10, 60);

        // Medical School
        doc.text(`Medical School: ${item.medSchool}`, 10, 70);

        // Research Score
        doc.text(`Research Productivity Score: ${item.researchScore}`, 10, 80);

        // Overall score
        doc.text(`Overall Score (Programme's Preference Score): ${item.prefferedRating}`, 10, 90);

        // SOP Score
        doc.setFont("Helvetica", "bold");
        doc.text("SOP Factor's Score:", 10, 100);
        doc.setFont("Helvetica", "normal");
        if(item.sopScore!=="NA"){
          doc.text(`- SOP Score: ${item.sopScore.percentageMatchInSOP}`, 10, 110);
        }else{
          doc.text(`- SOP Score: NA`, 10, 110);
        }
        // LOR Score
        doc.setFont("Helvetica", "bold");
        doc.text("LOR Factor's Score:", 10, 120);
        doc.setFont("Helvetica", "normal");
        if(item.lorScore!=="NA"){
          doc.text(`- LOR Score: ${item.lorScore.percentageMatchInLOR}`, 10, 130);
        }else{
          doc.text(`- LOR Score: NA`, 10, 130);

        }
        // Number of Letters of Recommendation
        doc.text(`Number of Letters of Recommendation: ${item.lor.length}`, 10, 140);
        ...
        if(item.lorScore!="NA"){
        doc.setFont("Helvetica", "bold");
        doc.text("LOR Characteristics:", 10, 20);
        doc.setFont("Helvetica", "normal");
        ...
        let charY = 30;
          item.lorScore.characteristicsInLOR.forEach((char, index) => {
              ...
              doc.text(`${index + 1}. ${char}`, 20, charY);
              charY += 10;

              // Add a new page if the content exceeds the page height
              if (charY >= pageHeight) {
                  doc.addPage();
                  charY = 20; // Reset Y for new page
              }
          });

        // // Similarity to Program’s Defined Characteristics
        // doc.setFont("Helvetica", "normal");
        // charY += 10;
        // if (charY >= pageHeight) {
        //     doc.addPage();
        //     charY = 20;
        // }
        // doc.text(`Similarity to Program’s Defined Characteristics: ${item.prefferedRating}%`, 10, charY);

        // New Page for Personal Statement (SOP)
        doc.addPage();
      }
        // Personal Statement (SOP)
        doc.setFont("Helvetica", "bold");
        doc.text("Personal Statement (SOP):", 10, 20);
        doc.setFont("Helvetica", "normal");
        doc.setFontSize(10);
        let sopY = 30;
        ...
        // Two-sentence summary (Assuming generateSummary function is defined)
        doc.setFontSize(12);
        sopY += 10;
        if (sopY >= pageHeight) {
            doc.addPage();
        ...
        doc.text("Summary:", 10, sopY);
        const summary = generateSummary(item.sop);
        sopY += 10;
        if (sopY >= pageHeight) {
            doc.addPage();
            sopY = 20;
        ...
        {result.length > 0 && (
          ...
          <div className={styles.sce}>
            ...
            <tr  key={index}>
              ...
              <td className={styles.mainHeader3}>{item.prefferedRating}{item.prefferedRating!="NA"?"%":""}</td>
              <td className={styles.mainHeader3}> <a href="#" onClick={()=>openPDFInNewTab(item)}>Report</a></td>
            </tr>
